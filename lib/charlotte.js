(function() {
  ...
  charlotte.createBrowser = function(options){
    ...
    function wrapCallbackWithErrorHandlers(tab, callback, page) {
      return function() {
    ...
    function request(settings, callback, renderWait, tab, page) {
      settings = clone(settings, PAGE_REFERENCE_PROPS);
              ...
              renderTemplate(bundle.version, layoutPath, bundle.template, req,
                ...
                function(err, html) {
                  ...
                  if (readyRegistry[requestId]) {
                    triggerReady = createTriggerReadyFunc(requestId, bundle.version, tab ? page : browser);
                  } else {
        ...
        load: function(page, triggerReadyCallback, isReload) {
          ...
          function handleRedirect(err, page) {
            if (err instanceof RedirectError && followRedirects) {
              ...
              if ((parsedCurrent &&
                ...
                var newPage = clone(page, PAGE_REFERENCE_PROPS);
          ...
          function loadView() {
            var viewOnlyPage = _.extend({
                ...
                clone(page, PAGE_REFERENCE_PROPS)),
            ...
            async.waterfall([
              function(next) {
                page.requestId = viewRequestId = request(viewOnlyPage, next, null, self, viewOnlyPage);
              },
              ...
              var callback = page.onViewLoad.callback || defaultCallback,
                  wrapper;
              viewLoaded = true;
              wrapper = wrapLoadCallback(page.onError, callback);
          ...
          function loadEverything() {
            var loadPage = clone(page, PAGE_REFERENCE_PROPS),
