(function() {
  var charlotte,
      ...
      find = function(selector) {
        return $(this.contentContainer, this.container).find(selector);
      },
      on = function() {
        var z = $(this.contentContainer, this.container)
  ...
  charlotte.createBrowser = function(options){
    ...
    function createTab(options) {
      ...
      tab = {
        ...
        load: function(settings, triggerReadyCallback, isReload) {
          ...
          var self = this,
              ...
              renderWait,
              lastSettings = self.length() > 0 ? self.current() : null,
          ...
          isPost = settings.type.match(/post/);
          this.pendingLength = this.length() + (isReload || isPost ? 0 : 1);
          shouldLoadView = settings.onViewLoad && settings.type.match(/get/) && !isReload;
          ...
          function archiveExistingContent() {
            ...
            if (existingContent) {
              ...
              if (lastSettings.archiver) lastSettings.archiver.onArchive(existingContent);
              existingContent.parentNode.removeChild(existingContent);
            }
          }
          
          function handleRedirect(err, settings, stageCtr) {
            if (err instanceof RedirectError && followRedirects) {
              ...
              if (current) {
                ...
                if (current.requestedSettings)
                  parsedRequestedCurrent = shared.parseUrl(current.requestedSettings.url);
              }
              if (previous) {
                parsedPrevious = shared.parseUrl(previous.url);
                if (previous.requestedSettings)
                  parsedRequestedPrevious = shared.parseUrl(previous.requestedSettings.url);
              }
              stageCtr.parentNode.removeChild(stageCtr);
              if ((parsedCurrent &&
                ...
                self.back(null, function() { self.reload() });
              } else {
                var newSettings = clone(settings);
                newSettings.requestedSettings = settings;
                delete newSettings.requestedSettings.viewOnly;
                delete newSettings.data
                delete newSettings.viewOnly;
                newSettings.type = 'get';
                newSettings.url = err.location;
                newSettings.isRedirectFromPost = isPost;
                self.load(newSettings);
              }
              return true;
            } else {
              return false;
          ...
          function pushOnStack() {
            stack.push(settings);
            if (_.isFunction(archiver)) archiver = archiver(tab, settings);
            settings.archiver = archiver;
            settings.index = stack.length-1;
          }
          ...
          function loadView() {
            ...
            async.waterfall([
              ...
              if (handleRedirect(err, viewOnlySettings, viewStageCtr)) return;
          ...
          function loadEverything() {
            ...
            settings.requestId = loadRequestId = request(loadSettings, 
              function(err, bundle, html, triggerReady) {
                ...
                if (handleRedirect(err, loadSettings, loadStageCtr)) return;
              ...
              async.waterfall([
                function(next) {
                  ...
                  if (!transition || isReload) {
                    ...
                    transition(contentCtr, loadStageCtr, archiveExistingAndCleanReplaced, viewLoaded);
                  }
                }
              ], function(err) {
          ...
          if (stack.length > 0) {
            if (settings.uploadOptions) {
              ...
              settings.uploadOptions.params['X-Referer'] = self.current().url;
            } else {
              settings.headers = settings.headers || {};
        ...
        back: function(options, callback) {
          var poppedSettings, lastSettings, onBack, transition, contentCtr;
          ...
          if (lastSettings.archiver) lastSettings.archiver.onRestore(lastSettings.content);
          async.waterfall([
            function(next) {
              if (transition) {
        ...
        reload: function(callback, isRedirectFromPost, newUrl) {
          ...
          $(contentCtr).off()
          _.each([
            settings.onDestroy, this.onDestroy, this.browser.onDestroy
          ], function(onDestroy) {
            if (onDestroy) onDestroy(contentCtr);
          });
          this.load(_.extend({isRedirectFromPost: isRedirectFromPost}, 
                      settings.requestedSettings ? settings.requestedSettings : settings), 
                    callback, true);
        },
      ...
      tab.isATab = true;
      _.each(['transitions', 'archiver'], function(option) {
        if (!tab[option]) tab[option] = eval(option);
      });
