(function() {
  ...
  function getResourceCacheFile(rootUrl, version, url, filePath, compileData, timeout, onCacheMiss, getServerResource, callback) {
    ...
    async.waterfall([
      ...
      function(next) {
        var url = shared.removeUrlMultislashes(charlotte.cacheSeedLocation + '/' + filePath);
        console.log("Requesting cache resource " + url)
        ...
        async.waterfall([
         ...
         function(foundInCacheSeed, fullPath, data, compiled, next) {
           if (foundInCacheSeed) {
             console.log("Found resource in cache seed: " + url)
             if (data) {
        ...
        ], function(err, fullPath, data, compiled, version) {
          // update cache items
