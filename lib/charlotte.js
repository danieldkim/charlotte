(function() {
  var charlotte,
      ...
      ASSET_METHOD_NAMES = ['assets', 'stylesheets', 'javascripts', 'require', 'assetUrls'],
  ...
  if (this.charlotte) {
    ...
    if (_.keys(this.charlotte).join() != 'shared') {
      ...
      modules = this.charlotte.modules;
      ...
      fileFullPathRoot = this.charlotte.fileFullPathRoot;
      fileUrlCache = this.charlotte.fileUrlCache;
      tempCacheSize = this.charlotte.tempCacheSize;
      contentContainer = this.charlotte.contentContainer;
      debug = this.charlotte.debug;
      resourceCache = this.charlotte.resourceCache;
    }
  }
  ...
  charlotte = this.charlotte = {
    ...
    fileFullPathRoot: fileFullPathRoot,
  ...
  function getAssetCacheFile(rootUrl, version, url, compileData, timeout, onCacheMiss, callback) {
  ...
  function processStylesheet(rootUrl, version, url, data, timeout, onCacheMiss, callback) {
    var resourcesToGet = [];
    ...
    async.forEach(_.uniq(resourcesToGet), 
      function(url, callback) {
        // recurse
         getAssetCacheFile(rootUrl, version, url, null, timeout,
    ...
    async.waterfall([
      ...
      function(next) {
        ...
        async.map(urls, function(url, callback) {
          var cached = version && inNativeApp,
              mappedUrl;
          url = shared.resolveUrl(assetRootUrl, url);
          if (cached) {
            mappedUrl = getFileUrl(rootUrl, version, url, calcRelFilePathForAsset);
          } else {
            mappedUrl = url;
          }
          if ((mappedUrl.match(/\.js$/) && $('script[src="' + mappedUrl + '"]').length) ||
              (mappedUrl.match(/\.css$/) && $('link[href="' + mappedUrl + '"]').length)) {
            callback(null, null);
          } else if (cached) {
            getAssetCacheFile(rootUrl, version, url, null, timeout, onCacheMiss, callback);
          } else {
            callback(null, mappedUrl);
          }
        }, next);
      },
      
      function(mappedUrls, next) {
        mappedUrls = mappedUrls.filter(function(url) { return url; });
        async.series(
            ...
            if (_.isFunction(f)) {
              ...
              _.each(ASSET_METHOD_NAMES, function(name) {
                  versionedAssetMethodWrappers[name] = function(options, callback) {
                    return context[name].call(context, _.extend({}, options, {version: version}), callback);
                  };
                });
  ...
  charlotte.initFileSystemCache = function(callback) {
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0,
      function(fs) { 
        fileFullPathRoot = charlotte.fileFullPathRoot = fs.root.fullPath;
        callback();
      },
      function(err) {
        callback(new Error("Error requesting file system"));
      });
  };
  ...
  charlotte.addCompiledTemplates = function addCompiledTemplates(version, templates) {
    _.each(templates, function(template, url) {
      var data = _.isString(template) ? eval('(' + template + ')') : template;
      setResourceCacheItem(charlotte.rootUrl, version, url, data);
    });
  };

  charlotte.setTemplatesJs = function setTemplatesJs(version, url, callback) {
    var self = this;
    getAssetCacheFile(self.rootUrl, version, 
                      shared.versionAssetUrl(version, self.assetRootUrl + url), 
                      function(data) { return data; }, null, function() {}, 
                      function(err, fileUrl, data) {
                        self.addCompiledTemplates(version, eval('('+data+')'));
                        callback();
                      });
  };
    ...
    assets.call(this, options,
      ...
      function(url, callback) {
        ...
        script.onerror = function(e) {
          if (generateAssetLoadErrors) callback(new AssetLoadError(url));
          else callback(null, script);
        };
        if (internal) {  
          ajax(url, null, null, function(err, data) {
            if (err) return callback(err);
            script.setAttribute('__src', url);
            script.text = data;
  ...
  charlotte.assets = function(options, callback) {
    ...
    if(scriptOptions) {
      ...
      funcs.push(function(urls, next) {
        if (urls) allUrls = allUrls.concat(urls)
        self.javascripts.call(self, scriptOptions, next);
      });
    }
    async.waterfall(funcs, function(err, urls){
      callback(err, allUrls.concat(urls));
    });
  };
  
  function assetUrls(options) {
    var version = options.version || this.version,
        rootUrl = options.rootUrl || this.rootUrl,
        urls = options.urls,
        self = this;
    if (charlotte.inNativeApp && !fileFullPathRoot) {
      throw new Error("File system cache not initialized.");
    }
    return _.map(urls, function(url) {
      var resolvedUrl = shared.resolveUrl(
        self.assetRootUrl, shared.versionAssetUrl(version, url));
      if (charlotte.inNativeApp) {
        return getFileUrl(rootUrl, version, resolvedUrl, calcRelFilePathForAsset);
      } else {
        return resolvedUrl;
      }
    });
  ...
  charlotte.assetUrls = function(options) {
    return assetUrls.call(this, options);
  }

  charlotte.resolveModule = resolveModuleValue;
  ...
  charlotte.require = function(options, callback) {
    ...
    async.whilst(
      ...
      function(callback) {
        ...
        self.javascripts(javascriptsOptions, function(err, urls) {
          if (err) return callback(err);
          var newDependencies = [];
          _.each(unloadedDependencies, function(dep) {
            if (!modules[dep.name]) {
      ...
      function(err) {
        var callbackArgs, 
            allDependenciesCallback = options.allDependenciesCallback;
        if (err) {
          if (allDependenciesCallback) allDependenciesCallback(err);
          return callback(err);
        }
        callbackArgs = _.map(resolvedDependencyNames, function(name) { 
                         return resolveModuleValue(name); 
                       });
        callbackArgs.unshift(null);
        callback.apply(self, callbackArgs);
        if (allDependenciesCallback) {
          allDependenciesCallback(null, _.keys(encounteredDependencies));
        }
      });
  };
  ...
  charlotte.createBrowser = function(options){
    ...
    var browser,
        ...
        bundleResponders = options.bundleResponders || [],
        ...
        function(callback) {
          ...
          async.forEach(templatesLeftToRetrieve, 
            function(path, callback) {
              ...
              // if (shouldTryCache) {
              if (version) {
                template = getResourceCacheItem(rootUrl, version, url) 
                if (template) {
                  templates[path] = template;
                  callback();
                } else {
                  getAssetCacheFile(rootUrl, version, url, compile, timeout, onCacheMiss,
            ...
            function(err) {
              ...
              mergedOptions.partial = function(path, partialOptions) {
                var url = shared.resolveUrl(assetRootUrl, shared.versionAssetUrl(version, resolveExtname(path)[1])),
          ...
          async.parallel([
            ...
            function(callback) {
              async.forEach(_.uniq(inlineAssetsToRetrieve), function(url, callback) {
                getAssetCacheFile(rootUrl, version, url, 
    ...
    function request(settings, callback, renderWait, page) {
      ...
      async.waterfall([
        ...
        function(next) {
          ...
          function matchUrlOrSettings(matcher) {
            if (_.isFunction(matcher)) {
              return matcher(fullUrl, parsedFullUrl, settings);
            } else {
          ...
          function defaultAction() {
            var bundleResponder = _.find(bundleResponders, function(responder) {
              return matchUrlOrSettings(responder.matcher);
            });
            if (bundleResponder) {
              bundleResponder.respond(settings, function(err, bundle) {
                bundle.version = localVersion;
                next(err, null, bundle);
              });
            } else {
              onCacheMissOnce();
              if (page) {
          ...
          if (settings.bundle) {
            return next(null, null, settings.bundle);
          } 
          ...
          isTempCacheUrl = _.any(cachedBundleOptions.tempUrlMatchers, matchUrlOrSettings);
          ...
          if (settings.viewOnly) {
            ...
            viewOnlyMatcherKey = _.find(_.keys(viewOnlyUrlMatchers), 
                                        function(key) {
                                          return matchUrlOrSettings(viewOnlyUrlMatchers[key]);
                                        });
          ...
          } else if (_.any(cachedBundleOptions.urlMatchers, matchUrlOrSettings)) {
      ...
      tab = {
        ...
        clear: function() {
          _.each(stack, function(page) {
            ...
            $(page.container).off(); // prevent memory leak
        ...
        load: function(page, triggerReadyCallback, isReload) {
          ...
          _.each(ASSET_METHOD_NAMES, function(name){
            page[name] = function() {
              return charlotte[name].apply(this, arguments);
            }; 
          });
          ...
          page.isAPage = true;
          _.each(['load', 'reload', 'back', 'backTo'], function(method) {
        ...
        back: function(options, callback) {
          ...
          charlotteOptions = (options && options.charlotte) || {};
          ...
          _.each(lastPage.archivers, function(archiver) {
            if (archiver.onRestore) {
              archiver.onRestore(lastPage.container, options);
            }
          });
          async.waterfall([
            function(next) {
              if (charlotteOptions.passThrough) {
                return next();
              }
              if (transition) {
            ...
            function(next) {
              _.each(lastPage.archivers, function(archiver) {
                if (archiver.onRestoreAfterTransition) {
                  archiver.onRestoreAfterTransition(lastPage.container, options);
                }
              });
              _.each([
        ...
        backTo: function(url, options, callback) {
          var self = this,
              topPage = self.current(),
              currUrl;
          async.whilst(
            function() {
              return currUrl !== url;
            ...
            function(callback) {
              var charlotteOptions = {};

              currUrl = self.previous().url;
              if (currUrl !== url) {
                charlotteOptions.passThrough = true; 
              }
              self.back(_.extend({charlotte: charlotteOptions}, options), callback); 
            }, 
            callback || function() {});
        },
      ...
      loadTemplates: function(version, paths, callback) {
        ...
        async.forEach(paths, 
          function(path, callback) {
            ...
            getAssetCacheFile(self.rootUrl, version, url, 
