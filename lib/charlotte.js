(function() {
  ...
  charlotte.createBrowser = function(options){
    ...
    function createTab(options) {
      ...
      function getTransition(page, type) {
        var transition;
        if (page[type]) transition = page[type].transition;
        if (_.isUndefined(transition) && tab[type]) transition = tab[type].transition;
        if (_.isUndefined(transition) && browser[type]) transition = browser[type].transition;
        return transition;
      }
            ...
            async.waterfall([
              ...
              function(bundle, html, triggerReady, next) {
                ...
                if (!transition) {
                  ...
                  transition.call(viewOnlyPage, contentCtr, viewStageCtr, archiveExistingAndCleanReplaced);
                }
              }
              ...
              var callback = page.onViewLoad.callback || defaultCallback,
                  wrapper;
              viewLoaded = true;
              wrapper = wrapLoadCallback(page, callback, true);
              wrapper(err, bundle, triggerReadyWrapper, isReload);
              if (!err) loadEverything();
            }); 
          }

          function loadEverything() {
            var loadPage = clone(page),
                transition = getTransition(page, 'onLoad');
              ...
              async.waterfall([
                function(next) {
                  ...
                  if (!transition || isReload) {
                    ...
                    transition.call(loadPage, contentCtr, loadPage.container, archiveExistingAndCleanReplaced, viewLoaded);
                  }
                }
              ], function(err) {
