(function() {
  ...
  function getResourceCacheFile(rootUrl, version, url, filePath, compileData, timeout, onCacheMiss, getServerResource, callback) {
    ...
    async.waterfall([
      ...
      function(next) {
        ...
        console.log("Requesting cache resource " + cacheUrl)
        ...
        async.waterfall([
         ...
         function(foundInCacheSeed, fullPath, data, compiled, next) {
           if (foundInCacheSeed) {
             console.log("Found resource in cache seed: " + cacheUrl)
             if (data) {
  ...
  function assets(options, versionCallback, findExisting, onCacheMiss, addNew, callback) {
    var self = this,
        version = options.version || localStorage.getItem(this.getVersionKey()),
    ...
    async.waterfall([
      function(next) {
        if (version) next();
          ...
          self.getVersion(rootUrl, timeout, function(err, ver) {
            version = ver;
            next(err);
          });
        }
      ...
      callbackWrapper = wrapCallbackWithErrorHandlers(function() {
                            ...
                            if (err && err.bundle && err.bundle.locals.completeBundleProcess) {
                              processBundle(err.data, err.bundle, function() {
                                onRequestEnd(settings, page, err.bundle);
                                callback.apply(this, arguments);
                              });
                            } else {
                              onRequestEnd(settings, page, arguments[1]);
                              callback.apply(this, arguments);          
                            }        
                          }, page);
      ...
      async.waterfall([
        ...
        function(next) {
          ...
          function defaultAction() {
            onCacheMissOnce();
            ajaxMethod(url, settings, parseBundle, 
              function(err, data, compiled) {
                if (err) return next(err);
      ...
      tab = {
        ...
        clear: function() {
          _.each(stack, function(page) {
            _.each([
              page.destroy, this.onDestroy, this.browser.onDestroy
            ], function(onDestroy) {
        ...
        load: function(page, triggerReadyCallback, isReload) {
          ...
          if (existingContent) {
            ...
            $(existingContent).data('scrollPositions', scrollPositions);
          }
            ...
            if ((viewCallbackComplete || !shouldLoadView) && 
              ...
              async.waterfall([
                function(next) {
                  ...
                  var contentCtr = $(self.contentContainer, self.container)[0];
                  var shouldDestroyExistingInstead = shouldLoadView || isReload || isPost;
                  function archiveExistingAndCleanReplaced(err) {
