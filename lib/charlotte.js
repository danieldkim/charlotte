    ...
    function createTab(options) {
      ...
      tab = {
        ...
        load: function(settings, triggerReadyCallback, isReload) {
          ...
          this.pendingLength = this.length() + 
            (isReload || settings.type.match(/post/) ? 0 : 1);
              ...
              function(err, bundle, html, triggerReady) {
                if (err instanceof RedirectError && followRedirects) {
                  var parsedLocation = shared.parseUrl(err.location),
                      ...
                      parsedCurrent = current ? shared.parseUrl(current.url) : null,
                      previous = self.previous(),
                      parsedPrevious = previous ? shared.parseUrl(previous.url) : null;
                  loadInProgress = false;
        ...
        back: function(options) {
          ...
          callback = onBack ? onBack.callback : undefined;
