        ...
        load: function(settings, triggerReadyCallback, isReload) {
          ...
          if (existingContent) {
            ...
            $('.scrolled', existingContent).each(function() {
              console.log("a scrollTop: " + this.scrollTop)
              scrollTops[this.id] = this.scrollTop;
            });
            console.log("storing scrollTops" + JSON.stringify(scrollTops));
              ...
              function(err, bundle, html, triggerReady) {
                if (err instanceof RedirectError && followRedirects) {
                  var parsedLocation = shared.parseUrl(err.location),
                      parsedCurrent = shared.parseUrl(self.current().url),
                      parsedPrevious = shared.parseUrl(self.previous().url);
                  loadInProgress = false;
                  loadStageCtr.parentNode.removeChild(loadStageCtr);
                  if (parsedLocation.host === parsedCurrent.host &&
                      parsedLocation.path === parsedCurrent.path) {
                    self.reload(null, true, location);
                  } else if (parsedLocation.host === parsedPrevious.host &&
