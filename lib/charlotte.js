(function() {
  ...
  function getFileUrl(rootUrl, version, url, calcRelativePath) {
    var filePath = getFilePath(rootUrl, version, url, calcRelativePath),
        ...
        defaultUrl = "file://" + fileFullPathRoot + filePath;
    if (fileUrlCacheItem) {
      return fileUrlCacheItem;
    } else if (resourceIsBinary(url)) {
      if (_.include(cacheSeedBinaryFiles, filePath)) {
        return shared.removeUrlMultislashes(
          docRootFileUrlBase + '/' + cacheSeedLocation + '/' + filePath
        );
      } else {
        return defaultUrl;
      }
        ...
        async.waterfall([
         ...
         function(next) {
           if (resourceIsBinary(filePath)) {
             var fullPath, 
                 inCacheSeed = _.include(cacheSeedBinaryFiles, filePath);
             if (inCacheSeed) {
               fullPath = shared.removeUrlMultislashes(
                 docRootFileUrlBase + '/' + cacheSeedLocation + '/' + filePath
                );
             } else {
               fullPath = fileFullPathRoot + filePath;
             }
  ...
  function assets(options, versionCallback, findExisting, onCacheMiss, addNew, callback) {
    var version = options.version || localStorage.getItem(this.versionKey),
