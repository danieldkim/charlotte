(function() {
  ...
  function restoreScrollTops(el) {
    if ($(el).data('scrollTops')) {
      _.each($(el).data('scrollTops'), function(scrollTop, id) {
        $('#' + id, el)[0].scrollTop = scrollTop
      });
    }    
  }
  ...
  pagetransitions.swap = function(contentCtr, contentStageCtr, options) {
    ...
    restoreScrollTops(contentStageCtr);
